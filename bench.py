...
E2FGVIHD_KERNEL_TYPE = "e2fgvihd_clean"
AUTOKERNEL_CACHE_DIR = Path.home() / ".cache" / "autokernel"
BASELINES_PATH = AUTOKERNEL_CACHE_DIR / "baselines.json"
...
# =========================================================================
# 8. E2FGVIHD WORKLOAD BENCHMARK
# =========================================================================

def _stack_numpy_output(output: Any) -> np.ndarray:
    if isinstance(output, np.ndarray):
        return output.astype(np.float32, copy=False)
    return np.stack([np.asarray(frame) for frame in output], axis=0).astype(np.float32)


def _load_e2fgvihd_manifest() -> dict[str, Any]:
    if not BASELINES_PATH.exists():
        raise FileNotFoundError(
            f"Baseline manifest not found: {BASELINES_PATH}. Run `uv run prepare.py` first."
        )
    ...
    for entry in baselines.values():
        if entry.get("workload") == E2FGVIHD_KERNEL_TYPE:
            return entry

    raise KeyError(
        f"No {E2FGVIHD_KERNEL_TYPE} baseline found in {BASELINES_PATH}. Run `uv run prepare.py` first."
    )


def _load_e2fgvihd_arrays(manifest: dict[str, Any]) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    data_dir = Path(manifest["test_data_dir"])
    frames_path = Path(manifest.get("frames_path", data_dir / "frames.npy"))
    masks_path = Path(manifest.get("masks_path", data_dir / "masks.npy"))
    baseline_output_path = Path(manifest["baseline_output_path"])

    missing = [p for p in (frames_path, masks_path, baseline_output_path) if not p.exists()]
    if missing:
        missing_str = ", ".join(str(p) for p in missing)
        raise FileNotFoundError(f"Missing cached E2FGVIHD artifact(s): {missing_str}")

    return np.load(frames_path), np.load(masks_path), np.load(baseline_output_path)


def _compare_numpy_output(
    ...
    if candidate.shape != baseline.shape:
        return {
            "allclose": False,
            "reason": f"shape mismatch: candidate={candidate.shape}, baseline={baseline.shape}",
            "max_abs_diff": float("inf"),
            "mean_abs_diff": float("inf"),
            "rmse": float("inf"),
        }

    candidate_f = candidate.astype(np.float32, copy=False)
    baseline_f = baseline.astype(np.float32, copy=False)
    abs_diff = np.abs(candidate_f - baseline_f)
    allclose = bool(np.allclose(candidate_f, baseline_f, atol=atol, rtol=rtol))
    return {
        "allclose": allclose,
        "reason": "" if allclose else f"max_abs_diff={float(abs_diff.max()):.6e} exceeds tol(atol={atol}, rtol={rtol})",
...
def run_e2fgvihd_benchmark(kernel_fn: Callable, gpu: GPUSpec, quick: bool = False) -> None:
    t_start = time.time()
    manifest = _load_e2fgvihd_manifest()
    frames, masks, baseline_output = _load_e2fgvihd_arrays(manifest)
    atol = float(manifest.get("allclose_atol", 1e-3))
    rtol = float(manifest.get("allclose_rtol", 1e-5))
    warmup = 1
    repeat = 1
    ...
    print(f"num_frames: {manifest.get('num_frames')}")
    print(f"frames_shape: {frames.shape}")
    print(f"masks_shape: {masks.shape}")
    print(f"baseline_output_shape: {baseline_output.shape}")
    print(f"allclose_atol: {atol}")
    print(f"allclose_rtol: {rtol}")

    print(f"\n=== CORRECTNESS + PERFORMANCE ===")
    for i in range(warmup):
        t0 = time.perf_counter()
        _ = kernel_fn(frames, masks)
    ...
    for i in range(repeat):
        if torch.cuda.is_available():
            torch.cuda.synchronize()
        t0 = time.perf_counter()
        candidate_output = kernel_fn(frames, masks)
        if torch.cuda.is_available():
            torch.cuda.synchronize()
        elapsed = time.perf_counter() - t0
        times.append(elapsed)
        print(f"[run {i + 1}] {elapsed:.3f}s  ({len(frames) / elapsed:.2f} fps)")

    candidate_array = _stack_numpy_output(candidate_output)
    cmp = _compare_numpy_output(candidate_array, baseline_output, atol=atol, rtol=rtol)
    correctness = "PASS" if cmp["allclose"] else "FAIL"
    best_time = min(times)
    mean_time = sum(times) / len(times)
    latency_us = best_time * 1e6
    latency_ms = best_time * 1e3
    fps = len(frames) / best_time
    baseline_latency_s = float(manifest.get("latency_s", 0.0) or 0.0)
    speedup_vs_baseline = baseline_latency_s / best_time if baseline_latency_s > 0 else 0.0
    peak_vram_mb = torch.cuda.max_memory_allocated() / 1024**2 if torch.cuda.is_available() else 0.0
    bench_time_seconds = time.time() - t_start

    print(f"\n--- E2FGVIHD Summary ---")
    print(f"correctness: {correctness}")
    print(f"allclose: {str(cmp['allclose']).lower()}")
    if cmp["reason"]:
        print(f"allclose_reason: {cmp['reason']}")
    print(f"max_abs_diff: {cmp['max_abs_diff']:.6f}")
    print(f"mean_abs_diff: {cmp['mean_abs_diff']:.6f}")
    print(f"rmse: {cmp['rmse']:.6f}")
    print(f"latency_s: {best_time:.6f}")
    print(f"latency_ms: {latency_ms:.3f}")
    print(f"latency_us: {latency_us:.2f}")
    print(f"latency_mean_s: {mean_time:.6f}")
    print(f"fps: {fps:.4f}")
    print(f"peak_vram_mb: {peak_vram_mb:.1f}")
    print(f"speedup_vs_baseline: {speedup_vs_baseline:.3f}x")

    print(f"\n=== FINAL ===")
    print(f"kernel_type: {E2FGVIHD_KERNEL_TYPE}")
    print(f"correctness: {correctness}")
    print(f"allclose: {str(cmp['allclose']).lower()}")
    print(f"max_abs_diff: {cmp['max_abs_diff']:.6f}")
    print(f"mean_abs_diff: {cmp['mean_abs_diff']:.6f}")
    ...
    print(f"peak_vram_mb: {peak_vram_mb:.1f}")
    print(f"speedup_vs_baseline: {speedup_vs_baseline:.3f}x")
    print(f"speedup_vs_pytorch: {speedup_vs_baseline:.3f}x")
    print(f"throughput_tflops: 0.000")
    print(f"pct_peak_compute: 0.0%")
    print(f"bench_time_seconds: {bench_time_seconds:.1f}")


# =========================================================================
# 9. MAIN -- orchestrate everything and produce structured output
# =========================================================================
...
def main():
    ...
    if kernel_type == E2FGVIHD_KERNEL_TYPE:
        try:
            run_e2fgvihd_benchmark(kernel_fn, detect_gpu(), quick=args.quick)
        except Exception as e:
            print(f"\nFATAL: E2FGVIHD benchmark crashed: {type(e).__name__}: {e}")
            traceback.print_exc()
            print(f"\n=== FINAL ===")
            print(f"kernel_type: {E2FGVIHD_KERNEL_TYPE}")
            ...
            print(f"throughput_tflops: 0.000")
            print(f"bench_time_seconds: {time.time() - t_start:.1f}")
            sys.exit(1)
        return
